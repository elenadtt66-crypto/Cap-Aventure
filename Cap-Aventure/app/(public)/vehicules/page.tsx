          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 stagger-children">
              {filteredVehicles.map((veh) => {
                ...
                const fuelIcon = veh.techSpecs?.fuel === 'Électrique' ? '⚡' : veh.techSpecs?.fuel === 'Hybride' ? '🔋' : '⛽';
                return (
                  <div 
                    key={veh.id}
                  ...
                  >
                    {/* Photo */}
                    <div className="relative h-56 overflow-hidden bg-brand-hover flex-shrink-0">
                      <img 
                      ...
                      {/* Prix flottant */}
                      <div className="absolute top-3.5 right-3.5 bg-white/95 backdrop-blur-md px-3 py-1 rounded-xl shadow-md border border-white/30">
                        <span className="text-sm font-extrabold text-brand-navy font-mono">{veh.pricePerDay}€</span>
                        <span className="text-[10px] text-brand-muted font-normal">/j</span>
                      </div>

                      {/* Propriétaire + note en bas */}
                      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                        <div className="flex items-center space-x-1.5 bg-white/90 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/20 shadow-sm">
                          <img 
                            src={veh.owner.avatar} 
                            alt={veh.owner.name} 
                            className="w-5 h-5 rounded-full object-cover"
                          />
                          <span className="text-[10px] font-bold text-brand-navy truncate max-w-[100px]">{veh.owner.name}</span>
                        </div>
                        <div className="flex items-center space-x-1 bg-brand-accent/90 backdrop-blur-md px-2 py-1 rounded-full">
                          <Star className="w-3 h-3 fill-white text-white" />
                          <span className="text-[10px] font-extrabold text-white">{veh.rating.toFixed(1)}</span>
                          <span className="text-[9px] text-white/70">({veh.reviewCount})</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
                      <div className="space-y-2">
                        <div className="flex items-center text-xs text-brand-muted">
                          <MapPin className="w-3.5 h-3.5 mr-1 text-brand-accent flex-shrink-0" />
                          <span className="truncate font-semibold">{veh.location}</span>
                        </div>

                        <h3 className="text-base font-extrabold text-brand-text group-hover:text-brand-accent transition-colors duration-200 leading-snug">
                          {veh.name}
                        </h3>
                      ...
                      <div className="grid grid-cols-4 gap-1.5 py-3 border-y border-brand-border">
                        <div className="flex flex-col items-center p-1.5 bg-brand-beige rounded-xl text-center">
                          <Users className="w-3.5 h-3.5 text-brand-muted mb-0.5" />
                          <span className="text-[10px] font-extrabold text-brand-text">{veh.seats}</span>
                          <span className="text-[9px] text-brand-muted">places</span>
                        </div>
                        <div className="flex flex-col items-center p-1.5 bg-brand-beige rounded-xl text-center">
                          <Bed className="w-3.5 h-3.5 text-brand-muted mb-0.5" />
                          <span className="text-[10px] font-extrabold text-brand-text">{veh.beds}</span>
                          <span className="text-[9px] text-brand-muted">couchages</span>
                        </div>
                        <div className="flex flex-col items-center p-1.5 bg-brand-beige rounded-xl text-center">
                          <span className="text-sm mb-0.5">{fuelIcon}</span>
                          <span className="text-[9px] font-extrabold text-brand-text truncate w-full text-center">{veh.techSpecs?.fuel || 'Diesel'}</span>
                          <span className="text-[9px] text-brand-muted">énergie</span>
                        </div>
                        <div className="flex flex-col items-center p-1.5 bg-brand-beige rounded-xl text-center">
                          <Zap className="w-3.5 h-3.5 text-brand-muted mb-0.5" />
                          <span className="text-[9px] font-extrabold text-brand-text">{veh.techSpecs?.transmission === 'Automatique' ? 'Auto' : 'Manue'}</span>
                          <span className="text-[9px] text-brand-muted">boîte</span>
                        </div>
                      </div>
                      ...
                      {veh.features && veh.features.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {veh.features.slice(0, 3).map((f) => (
                            <span key={f} className="inline-flex items-center px-2 py-0.5 bg-brand-gold-light text-brand-accent rounded-full text-[10px] font-semibold">
                              ✓ {f}
                            </span>
                          ))}
                          {veh.features.length > 3 && (
                            <span className="inline-flex items-center px-2 py-0.5 bg-brand-hover text-brand-muted rounded-full text-[10px] font-semibold">
                              +{veh.features.length - 3}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2.5 pt-2">
                        <Link
                          href={`/vehicule/${veh.slug || veh.id}`}
                          className="flex-1 flex items-center justify-center space-x-1.5 py-2.5 border border-brand-border text-brand-text hover:border-brand-accent/50 hover:text-brand-accent bg-brand-beige hover:bg-brand-hover rounded-xl text-xs font-bold transition-all duration-200"
                        >
                        ...
                        <Link
                          ...
                          className="flex-1 flex items-center justify-center space-x-1.5 py-2.5 bg-brand-accent hover:bg-brand-accent-hover text-white rounded-xl text-xs font-bold shadow-sm btn-transition"
                        >
                          <span>Réserver</span>
