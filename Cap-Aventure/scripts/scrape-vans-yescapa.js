...
function generateVanDetails(name, location, price, rating) {
  ...
  // Sélectionner 5 à 7 caractéristiques aléatoires mais pertinentes
  ...
  // Générer des images additionnelles de qualité
...
async function runScraper() {
  ...
  // Scraper plusieurs pages pour avoir une grosse liste de Vans réels Yescapa
  const pagesToScrape = [
  ...
  for (let i = 0; i < pagesToScrape.length; i++) {
    ...
    try {
      ...
      for (const item of items) {
        const id = `yescapa-van-${item.identifier || Date.now()}-${Math.floor(Math.random()*1000)}`;
        const name = item.name === 'Camping-car Profilé' ? `Van Aménagé ${item.offers?.availableAtOrFrom?.address?.addressLocality || 'Aventure'}` : item.name;
        ...
        const uniqueKey = name + location + price;
        if (namesSet.has(uniqueKey)) continue;
        namesSet.add(uniqueKey);

        const details = generateVanDetails(name, location, price, rating);
        const mainImage = item.image ? item.image.replace(/&twic=v1\/cover=\d+x\d+/g, '') : 'https://images.unsplash.com/photo-1523987355523-c7b5b0dd90a7?auto=format&fit=crop&w=1200&q=80';
        ...
        const vanObj = {
          ...
          location: location,
          owner: {
            name: ['Marc & Sophie', 'Antoine B.', 'Élodie', 'Thomas & Clara', 'Laurent G.'][Math.floor(Math.random() * 5)],
  ...
  // Placer les nouveaux Vans au début de la liste pour qu'ils soient directement visibles
